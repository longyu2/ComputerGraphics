...
const animatePlay = (value) => {
  ...
  const timeSetInterval = setInterval(() => {
    ...
    for (let i = 0; i < (360 / jiao) * 2; i++) {
      ...
      let B1 = {
        x: xyToComputerXy(prevPoint.x, prevPoint.y, 1000, 1000, 1).x,
        y: xyToComputerXy(prevPoint.x, prevPoint.y, 1000, 1000, 1).y,
      };
      let B2 = {
        x: xyToComputerXy(x1, y1, 1000, 1000, 1).x,
        y: xyToComputerXy(x1, y1, 1000, 1000, 1).y,
      };
    ...
    if (value === "1") {
      ...
      // 减少大小,角的数量减少
      if (
        size * Math.tan((((2 * Math.PI) / 360) * jiao) / 2) > 80 &&
        jiao < 60
      ) {
        size -= 10;
        jiao += 0.1;
