...
const ground = (offset) => {
  for (let i = -90; i < 90; i += 8) {
    ...
    ctx.lineWidth = 0.2
    let [xcx, xcy] = [project3Dto2D (i - offset.x, -30 - offset.y, 30 - offset.z, focalLength), project3Dto2D (i - offset.x, -30 - offset.y, 225 - offset.z, focalLength)]


    drawLine(xyToComputerXy(xcx.x, xcx.y, 15, 15, 66), xyToComputerXy(xcy.x, xcy.y, 15, 15, 66), ctx)

    let [ycx, ycy] = [project3Dto2D (-90 - offset.x, -30 - offset.y, i + 140 - offset.z, focalLength), project3Dto2D (90 - offset.x, -30 - offset.y, i + 140 - offset.z, focalLength)]
...
const renderFrame = () => {
  ...
  for (let i = 0; i < moedels.length; i++) {
    drawFrame(moedels[i], distanceZ, angle, focalLength, ctx, offset, zBufferArr);
  }
